...
The easiest way to install { siteConfig.customFields.appName } on { props.label } is by using the **binary xPack**, available as
<a href={ `https://www.npmjs.com/package/@xpack-dev-tools/${ siteConfig.customFields.appLcName}` }><code>@xpack-dev-tools/{ siteConfig.customFields.appLcName }</code></a>
...
Upon initial use, ensure that a `package.json` file is present in the project.
This can be achieved by running `xpm init` in the project folder
(substitute `my-project` with the name of your project):
...
**{ siteConfig.customFields.appLcName }** package in the project:

<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/${ siteConfig.customFields.appLcName }@latest --verbose`
}</CodeBlock>
...
* install the latest available version of { siteConfig.customFields.appName }
...
* add <code>@xpack-dev-tools/{ siteConfig.customFields.appLcName }</code> to `package.json` as a
  development dependency; this associates a specific version of { siteConfig.customFields.appName } with
...
it is also possible to install { siteConfig.customFields.appName } only globally in the **central
xPacks store**, without any local
{ props.isWindows === 'True' ? (<><code>.cmd</code> forwarders</>) : 'symbolic links' },
assuming the tools are capable of handling
the paths themselves.

:::note

Installing packages locally into a project always installs the
packages in the **central xPacks store**, and subsequent attempts to
install the package globally will fail with _already installed_.

:::

<CodeBlock language="sh"> {
`xpm install --global @xpack-dev-tools/${ siteConfig.customFields.appLcName }@latest --verbose`
}</CodeBlock>
