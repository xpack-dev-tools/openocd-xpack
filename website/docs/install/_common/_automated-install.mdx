...
Instead, the binary packages can be installed in a platform-dependent folder
...
**xpm** adds { props.isWindows === 'True' ? (<><code>.cmd</code> forwarders</>) : 'symbolic links'}
...
but it avoids wasting disk space.
...
without creating local { props.isWindows === 'True' ? (<><code>.cmd</code> forwarders</>) : 'symbolic links'}
(see below).
...
**{ siteConfig.customFields.appLcName }** package into the project:
...
* identify the platform specific archive for the latest available version
  of { siteConfig.customFields.appName }, download it in a cache and unpack
