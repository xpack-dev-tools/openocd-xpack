...
The easiest way to install { customFields.appName } is by using the **binary xPack**, available as
<a href="https://www.npmjs.com/package/@xpack-dev-tools/{ customFields.appLcName }"><code>@xpack-dev-tools/{ customFields.appLcName }</code></a>
...
then install the **{ customFields.appLcName }** package in the local project:

<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/${ customFields.appLcName }@latest --verbose`
}</CodeBlock>
...
* install the latest available version of { customFields.appName }
...
* add <code>@xpack-dev-tools/{ customFields.appLcName }</code> to `package.json` as a
  development dependency; this associates a specific version of { customFields.appName } with
...
it is also possible to install { customFields.appName } only globally in the **central
...
<CodeBlock language="sh"> {
`xpm install --global @xpack-dev-tools/${ customFields.appLcName }@latest --verbose`
}</CodeBlock>
