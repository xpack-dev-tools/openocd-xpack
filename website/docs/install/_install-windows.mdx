...
<code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\openocd\\xpack-openocd-{ customFields.upstreamVersion }-{ customFields.xpackSubversion }</code> folder
(for example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\openocd\\xpack-openocd-{ customFields.upstreamVersion }-{ customFields.xpackSubversion }</code>);
